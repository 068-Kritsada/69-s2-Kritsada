...
module.exports = {
  ...
  async forgotPassword(ctx) {
    ...
    if (!user || user.blocked) {
      return ctx.send({ ok: true, resetPasswordToken: null });
    }
    ...
    ctx.send({ ok: true, resetPasswordToken });
  },
    ...
    const allowedKeys = compact(
      concat(
        ...
        isArray(register?.allowedFields)
          ? // Note that we do not filter allowedFields in case a user explicitly chooses to allow a private or otherwise dangerous user schema field
          ...
          : // to prevent breaking changes, if allowedFields is not set in config, we only remove private and known dangerous user schema fields
            ...
            Object.keys(attributes).filter(
              (key) =>
                ...
                ![
                  // many of these are included in nonWritable, but we'll list them again to be safe
